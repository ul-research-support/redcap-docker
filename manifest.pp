...
#Installs and runs Httpd web server
class httpd {
   package { 'httpd':
      ensure  => present,
...
class extraction {
    ...
    exec { "move":
      command => "mv /var/www/html/redcap*/redcap /var/www/html",
      path => ["/usr/bin", "/bin"],
      require => Exec['extract'],
    }
    exec { "remove":
      command => "rm -rf /var/www/html/redcap7.0.15",
      path => ["/usr/bin", "/bin"],
      require => Exec['move'],
    }
}
...
define mysqlsetup1 {
   exec { "run_sql_setup1":
      command => "/usr/bin/mysql -u redcap_user -pMedcenter140b redcap < /var/www/html/redcap/redcap_v7.0.15/Resources/sql/install.sql",
...
define mysqlsetup2 {
   exec { "run_sql_setup2":
      command => "/usr/bin/mysql -u redcap_user -pMedcenter140b redcap < /var/www/html/redcap/redcap_v7.0.15/Resources/sql/install_data.sql",
      path    => ["/usr/local/bin/:/var/www/html/redcap/redcap_v*/Resources/sql"],
      require => Exec['flush'],
   }
}
...
exec { "update-redcap-db":
  command => "/usr/bin/mysql -u redcap_user -pMedcenter140b redcap -e \"UPDATE redcap_config SET value = 'sha512' WHERE field_name = 'password_algo';
...
\"",
  ...
  subscribe => [Service['httpd'], Service['mysqld']],
}
